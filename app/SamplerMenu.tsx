  ...
  const getSamplerList = (): APISampler[] => {
    ...
    if (activeIndex !== -1) {
      ...
      const template = getTemplates().find(
        (item) => item.name === currentApiValue.configName
      )
      if (!template) return []
    ...
    <FadeDownView style={{ flex: 1 }}>
      <TextBoxModal
        ...
        onConfirm={(text: string) => {
          ...
          if (!currentConfig) {
            Logger.errorToast('Cannot add new sampler: current configuration is not loaded.')
            return
          }
        ...
        {currentConfig &&
          getSamplerList().map((item) => {
            ...
            switch (samplerItem.inputType) {
              case 'slider':
                if (samplerItem.values.type === 'float' || samplerItem.values.type === 'integer') {
                  return (
                    <ThemedSlider
                      key={item.samplerID}
                      value={currentConfig.data[samplerItem.internalID] as number}
                      onValueChange={(value) =>
                        updateCurrentConfig({
                          ...currentConfig,
                          data: { ...currentConfig.data, [samplerItem.internalID]: value },
                        })
                      }
                ...
                return (
                  <ThemedCheckbox
                    ...
                    onChangeValue={(b) =>
                      updateCurrentConfig({
                        ...currentConfig,
                        data: { ...currentConfig.data, [samplerItem.internalID]: b },
                      })
                    }
                    label={samplerItem.friendlyName}
                  />
                ...
                return (
                  <ThemedTextInput
                    ...
                    onChangeText={(text) =>
                      updateCurrentConfig({
                        ...currentConfig,
                        data: { ...currentConfig.data, [item.samplerID]: text },
                      })
                    }
                    label={samplerItem.friendlyName}
                  />
