...
export class TcpClient {
  ...
  private connectionStatusCallback: ((status: 'Connected' | 'Connecting...' | 'Disconnected' | 'Error') => void) | null = null;
  ...
  public setStatusCallback(callback: (status: 'Connected' | 'Connecting...' | 'Disconnected' | 'Error') => void) {
  ...
  async send(payload: Request): Promise<Response> {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        console.log(`[TcpClient] Compressed payload size: ${compressedData.length} bytes. Ratio: ${(compressedData.length / jsonString.length * 100).toFixed(2)}%`);
  ...
  private setupListeners(): void {
    ...
    const attachListeners = (s: TcpSocket.Socket) => {
      ...
      s.on('error', (error) => {
        ...
        s.emit('responseError', new Error(`Socket error during communication: ${error.message}`));
  ...
  private handleIncomingData(data: Buffer): void {
    ...
    while (this.receivedDataBuffer.length >= 4) { // Need at least 4 bytes for length prefix
      ...
      if (this.receivedDataBuffer.length >= 4 + messageLength) {
        ...
        try {
          ...
          this.socket?.emit('responseError', new Error(`Failed to parse response: ${e.message}`));
        }
