...
export const usePeerDiscovery = () => {
  ...
  const [discoveryStatus, setDiscoveryStatus] =
    useState<PeerDiscoveryStatus>('Initializing')
    ...
    if (Platform.OS === 'android') {
      try {
        ...
        const allGranted =
          ...
          (Platform.Version < 33 || granted['android.permission.NEARBY_WIFI_DEVICES'] === PermissionsAndroid.RESULTS.GRANTED)
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (groupInfo && groupInfo.groupFormed) {
          WiFiP2P.removeGroup().catch(e => Logger.error('Failed to remove P2P group on unmount:', e));
      }
  ...
  return {
    ...
    connectToPeer: async (peer: DiscoveredPeerInfo) => {
      ...
      try {
        ...
        Logger.info(`Connection request sent to ${peer.deviceName}. Waiting for connectionInfoUpdated event.`);
        ...
        setDiscoveryStatus('Error');
        Alert.alert('Connection Failed', `Could not connect to ${peer.deviceName}: ${e.message}`);
      }
    },
    removeGroup: async () => {
