...
const cohereRequest = (config: APIConfiguration, values: APIValues) => {
    ...
    if (!Array.isArray(promptData)) {
        Logger.errorToast(
            'Cohere Request: Prompt data is not an array for chat completion type. Returning undefined payload.'
        )
        return undefined
    }

    let preambleContent: string | undefined = undefined
    let chatHistoryMessages: { role: string; message: string }[] = []
    ...
    if (promptData.length > 0) {
        ...
        if (
            ...
            ('content' in firstMessage || 'message' in firstMessage)
        ) {
            ...
            Logger.warn(
                'Cohere request: First message in promptData has no valid content/message for preamble.'
            )
        }
        ...
        chatHistoryMessages = historySlice
            .map((msg) => {
                if (
                    msg &&
                    typeof msg === 'object' &&
                ...
                Logger.warn(
                    'Cohere request: Invalid message format in chat history. Skipping this message.'
                )
                return null // Return null for invalid messages, will be filtered out
            })
            .filter(Boolean) as { role: string; message: string }[] // Filter out nulls and assert type
        ...
        if (
            ...
            ('content' in lastMessage || 'message' in lastMessage)
        ) {
            // Prefer 'content', fallback to 'message' if present
        ...
        Logger.warn(
            'Cohere request: Prompt data array is empty after initial check for chat completion type. This should ideally not happen if buildChatCompletionContext ensures at least one message for chat.'
        )
