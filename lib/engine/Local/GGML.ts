...
function readUInt32LE(bytes: Uint8Array, offset: number): number {
  ...
  if (offset + 4 > bytes.length) {
    throw new Error(`[GGML] Insufficient data to read 32-bit integer at offset ${offset}. Array length: ${bytes.length}`);
  }
...
export async function getGGMLTypeFromFile(filePath: string): Promise<GGMLType> {
  try {
    ...
    const uint8Array = base64ToUint8Array(base64Content.substring(0, Math.ceil(bytesToRead / 3) * 4));
    ...
    let offset = 8;
    const tensorCount = readUInt32LE(uint8Array, offset); offset += 4;
    const kvCount = readUInt32LE(uint8Array, offset); offset += 4;
    ...
    for (let i = 0; i < kvCount; i++) {
        ...
        let valueSize = 0;
        switch (valueType) {
            case 0: valueSize = 1; break; // GGUF_TYPE_UINT8
            case 1: valueSize = 1; break; // GGUF_TYPE_INT8
            case 2: valueSize = 2; break; // GGUF_TYPE_UINT16
            case 3: valueSize = 2; break; // GGUF_TYPE_INT16
            case 4: valueSize = 4; break; // GGUF_TYPE_UINT32
            case 5: valueSize = 4; break; // GGUF_TYPE_INT32
            case 6: valueSize = 8; break; // GGUF_TYPE_UINT64
            case 7: valueSize = 8; break; // GGUF_TYPE_INT64
            case 8: valueSize = 4; break; // GGUF_TYPE_FLOAT32
            case 9: valueSize = 8; break; // GGUF_TYPE_FLOAT64
            case 10: valueSize = 1; break; // GGUF_TYPE_BOOL
            ...
            case 12: // GGUF_TYPE_ARRAY - length of array precedes elements
                ...
                break;
            default:
                console.warn(`[GGML] Unknown GGUF value type: ${valueType} for key ${key}. Skipping.`);
                break;
        }
        if (valueSize === 0) {
